...
后端图谱接口保持 `nodes` 和 `edges` JSON 结构，同时为节点补充章节标题、章节顺序和页码范围。前端通过 GraphPresentation 将同一份知识图谱数据转换为两种可视化：默认的章节思维导图按教材、章节和知识点分层规整排列；关系网络保留 Cytoscape 力导向布局，用于查看跨章节关系密度。两种视图共享搜索、高亮、缩放、拖拽和点击详情能力。
