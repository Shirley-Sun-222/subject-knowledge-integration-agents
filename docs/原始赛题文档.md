...
本文件保留 README 重构前的完整版本，用于归档当时的项目说明与赛题原文。

如果你只想查看当前项目的入口说明，请阅读根目录 [README.md](/Users/sunxueli/CodexFile/subject-knowledge-integration-agents/README.md:1)。

如果你只想查看按比赛要求整理的赛题说明，请从下方 “以下为原赛题文档” 开始阅读。
...
python3 scripts/run_rag_benchmark.py --question-set scripts/benchmark_sets/medical_official_questions.json
python3 scripts/run_p2_rag_experiments.py --sample-dir textbooks --mode all
```

`scripts/run_rag_benchmark.py` 默认内置 25 个算法/数据结构问题，也支持通过 `--question-set` 加载自定义题集。`scripts/benchmark_sets/medical_official_questions.json` 是面向官方医学教材的 25 题 benchmark。`scripts/run_p2_rag_experiments.py` 会在隔离运行时目录中导入 `textbooks/` 官方教材、执行四层消融和 chunk size 敏感性实验，并把结果写入 `data/generated/p2-rag-official/`。当前 benchmark 仍是自动化质量代理指标，最终提交前仍建议人工抽查回答准确率和引用准确率。
