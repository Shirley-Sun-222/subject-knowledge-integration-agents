...
def enqueue_integration(workspace_id: str = "global") -> tuple[dict, bool]:
    return task_runner.enqueue(
        workspace_id,
        "run_integration",
        "system",
        "global",
        lambda context: _run_integration_task(context, workspace_id=workspace_id),
    )


def _run_integration_task(context: TaskContext, workspace_id: str = "global") -> dict:
    context.start("aligning_nodes", progress_total=1)
    result = run_integration(progress=context, workspace_id=workspace_id)
    return {
...
def run_integration(progress: TaskContext | None = None, workspace_id: str = "global") -> dict:
    nodes = get_all_graph_nodes(workspace_id=workspace_id)
    original_chars = state_store.original_chars(workspace_id)
    groups = AlignmentAgent().group_nodes(nodes, workspace_id=workspace_id)
    ...
    state_store.replace_integration_decisions(workspace_id, decisions)
    if progress is not None:
        progress.progress(phase="writing_integration", progress_current=1, progress_total=1)
    return get_integration(stats, workspace_id=workspace_id)


def get_decisions(workspace_id: str = "global") -> list[dict]:
    return state_store.list_integration_decisions(workspace_id)


def update_decision(decision: dict, workspace_id: str = "global") -> None:
    state_store.update_integration_decision(workspace_id, decision)


def get_integration(stats: dict | None = None, workspace_id: str = "global") -> dict:
    nodes = get_all_graph_nodes(workspace_id=workspace_id)
    by_id = {node["id"]: node for node in nodes}
    decisions = get_decisions(workspace_id=workspace_id)
    integrated_nodes = []
    removed = set()
    ...
    all_edges = state_store.list_all_graph_edges(workspace_id)
